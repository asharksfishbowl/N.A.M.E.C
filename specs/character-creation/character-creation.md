...
3. A 3D preview of the character is visible on every step. The player can rotate it, zoom between full body and face, and toggle between three preview outfits: starting clothes (the Plain Shirt and Plain Trousers, Requirement 28), Leather armor, and Metal armor (`specs/character-progression/character-progression.md` Requirement 27).
...
### Starting Items
28. A new character starts with exactly two items, the Plain Shirt equipped in the Chest slot and the Plain Trousers equipped in the Legs slot (`specs/inventory/inventory.md` Requirement 12), and no other items. The starting item list (item definitions and their equipment slots) lives in `DT_Progression_Rules`. Both items are clothing with the Cloth armor category (`specs/character-progression/character-progression.md` Requirement 27), 0 Armor, 0 Insulation, 0 Cooling, and `Value` 0, so they cannot be sold to a Vendor (`specs/factions-kingdoms/factions-kingdoms.md` Requirement 32). Each defines 2 dye zones (Primary and Secondary; `specs/crafting-jobs/crafting-jobs.md` Requirement 20), and like every wearable they provide all 12 body variants (Requirement 18).
...
3. On Confirm, `UNamecCharacterFactory::CreateCharacter(Draft)` validates the name and option indexes, computes starting stats (10 + race + class), creates the character GUID, places the racial active ability in the last ability bar slot (Requirement 11), adds the starting items from `DT_Progression_Rules` equipped in their slots (Requirement 28), and writes a new `UNamecCharacterSave`.
