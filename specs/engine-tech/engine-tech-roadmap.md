...
5. Build tasks exist in `build-queue.groovy` for Phase 1 only. Later phases stay here until Phase 1 lands. **Waived for Phase 2 on 2026-09-19** (issue 0919-2, user direction): Phase 2 tasks are filed on `feature/foundation/base` while the Phase 1 benchmark run stays open as a recorded checkpoint, not a passed gate.
...
**Corrected 2026-09-19:** since 2026-09-18 the pipeline compiles (`Tools/Build/ubt.bat`), runs commandlets and runs `-nullrhi` automation through `UnrealEditor-Cmd.exe` on the reference PC over SSH, from `.worktrees/` checkouts, and merges a task once its proving command passes there. Only GPU-bound or editor-only steps remain user sittings: the task 4 lighting checkpoint (answered 2026-09-18), the editor authoring in `research/authoring-headless.md`, and the benchmark run. The rest of this paragraph is the 2026-09-16 plan, kept for the record. **Verification is entirely off-pipeline.** The container compiles, runs and measures nothing. Every task is authored as C++ and text, ends in "awaiting verification" with a handover note (`specs/authored-map/authored-map.md` Requirement 34), and merges into the parent branch only after the user pastes the result of the note's proving command. Budget about fifteen user sittings for Phase 1. Tasks 7–11 ship as C++ plus a written editor authoring recipe, because their uassets can only be authored on the reference PC.
...
| engine-tech | Evaluate: **CommonUI With Enhanced Input** (Requirement 5) — whether the settings UI and the per-local-player UI layer built in this phase trigger the 5.8 Experimental dispatcher path. Measured 2026-09-19 (`research/0919-2-phase-2-decomposition.md` §3.1): the Experimental path is the single project-wide flag `bEnableEnhancedInputSupport` in `CommonInputSettings`, default off; Phase 2 pins it `False` and proves the routing in the settings-screens task. The lobby half is evaluated in Phase 3 with multiplayer Requirement 4. Original wording: whether the settings and lobby UI built in this phase triggers the 5.8 Experimental dispatcher path (`research/unreal-5.6-5.8-features.md` §5.7); row added 2026-09-18 from the 09-18 research review |
...
**Superseded in part on 2026-09-18:** the container still compiles nothing itself, but it drives the reference PC over SSH — builds, commandlets and `-nullrhi` automation run there without the user — and pipeline branches do commit commandlet-generated uassets (DataTables, PCG graph skeletons). `UnrealEditor.exe` cannot start from the non-interactive SSH session, so PIE, Lumen checks and the benchmark stay with the user until the interactive launcher (`Tools/Build/interactive-launcher-watcher.ps1`) is registered (`research/gpu-runs-from-ssh.md`). Original decision follows. Nothing in any phase is compiled, run or measured in the pipeline container. The pipeline authors C++, ini, JSON, CSV-fed data tables and text inputs; the user compiles, runs automation tests and commandlets, opens PIE and runs the benchmark on the reference PC. Task 1 compiles before any sub-branch cuts and declares header-only stubs for every Key Files class so include paths and module dependencies are validated in one round trip. Each handover note lists the engine classes and functions the task assumed against Unreal 5.8 documentation, so a compile error maps to a stated assumption. Results flow back as pasted output; `Saved/Benchmark/<timestamp>/results.json` and `content-manifest.json` are transcribed into the Benchmark Results table by the Researcher.
