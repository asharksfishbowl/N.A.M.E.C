...
1. Nine Jobs exist, each a row in `DT_Crafting_Jobs`:
   ...
   - **Alchemist** — potions (healing, mana, resistance), poisons and weapon oils (consumables that coat the Right Hand weapon with a status buildup for a number of hits, `specs/combat-loot/combat-loot.md` Requirement 61), dyes (Requirement 18).
...
### Recipe Provenance
24. Every crafted item instance, including hand-crafted items (Requirement 16), stores the row ID of the `DT_Crafting_Recipes` row that crafted it. The recipe row ID is saved with the item instance (`specs/inventory/inventory.md` Data Flow 4), stays on the instance when it is dropped, picked up by a player or an enemy, or moved into or out of a container, and stays with the building piece placed from that item (`specs/voxel-world/voxel-world.md` Requirement 24). An item's **provenance recipe** is:
    - the stored recipe row, for an item instance that has one;
    - for an item instance with no stored recipe row ID (looted, bought from a Vendor, received as a quest reward, or a starting item, `specs/character-creation/character-creation.md` Requirement 28), the first `DT_Crafting_Recipes` row whose output is that item definition;
    - none, when no `DT_Crafting_Recipes` row outputs that item definition.
    
    The provenance recipe's input materials decide a deconstructed piece's refund (`specs/voxel-world/voxel-world.md` Requirement 27), a destroyed piece's material drop (`specs/voxel-world/voxel-world.md` Requirement 26), and a station repair's cost and station (`specs/combat-loot/combat-loot.md` Requirement 35). An item or piece with no provenance recipe refunds nothing when deconstructed, drops no materials when destroyed, and can be repaired only with a Repair Kit. Item instances of the same item definition with different stored recipe row IDs, or where one has a stored recipe row ID and the other has none, are separate inventory rows (`specs/inventory/inventory.md` Edge Case 13).

### Enchanting Screen
25. Interacting with an Enchanting Altar opens its station UI (Data Flow 1), which also holds the enchanting screen in the owning local player's viewport. On the enchanting screen, the player picks an item from the inventory (equipped or not), a rune from the inventory, and, for a reroll rune, one of the item's affixes, then applies the rune (Requirement 12, Data Flow 9). The world does not pause while a player uses the enchanting screen. The enchanting screen closes without applying the pending selection when the player becomes Downed, takes damage, moves out of the Enchanting Altar's interaction range (tuning value), or the Enchanting Altar is destroyed or deconstructed, like the dye screen (Requirement 23). Two or more players can use the same Enchanting Altar at the same time, each with an independent enchanting screen.
...
9. Player opens the enchanting screen at an Enchanting Altar (Requirement 25) → picks an item, a rune, and, for a reroll rune, an affix → client sends `ServerApplyRune(AltarId, ItemInstanceId, RuneInstanceId, AffixIndex)` (AffixIndex is ignored for an add rune) → server validates:
   - range: the player is within the Enchanting Altar's interaction range;
   - ownership: the item instance and the rune instance are in the player's inventory;
   - rune type vs action: an add rune performs an add, and a reroll rune performs a reroll, and for a reroll AffixIndex names an affix on the item;
   - affix cap: for an add, the item has fewer affixes than its rarity's max affix count, or is Common jewelry with 0 affixes (Requirement 12), and its slot's affix pool in `DT_Loot_Affixes` has an affix type not already on the item; for a reroll, that pool has an affix type not already on the item;
   - item eligibility: the item has a slot affix pool in `DT_Loot_Affixes` (equipment and jewelry; not consumables, materials, runes, dyes, or quest items).
   
   When validation fails, nothing is consumed and the item is unchanged. When it passes, the server removes the rune instance, rolls the affix type and value (Requirement 12) through `UNamecEnchantingService`, updates the item instance (raising Common jewelry to Magic on its first added affix), awards no Job XP, and replicates equipped item visuals when the item is equipped.
...
11. When the chosen item or the chosen rune leaves the player's inventory (dropped, sold, moved into a container, or consumed) while the enchanting screen is open, the server rejects `ServerApplyRune`, the rune is not consumed, and the item is unchanged.
12. When a player takes damage while the enchanting screen is open, the screen closes without applying the pending selection, as the dye screen does (Requirement 25).
13. When an item instance stores a recipe row ID that no longer exists in `DT_Crafting_Recipes` (content changed), the item still loads, and its provenance recipe is the first `DT_Crafting_Recipes` row whose output is its item definition, or none (Requirement 24).
...
- [ ] A character with Enchanter level 1 applies a tier-3 add rune at a tier-1 Enchanting Altar: the rune is consumed, an affix is added with a value capped by the tier-3 RuneMaxItemLevel, and the character's Enchanter XP is unchanged.
- [ ] `ServerApplyRune` is rejected, consuming nothing, for a reroll rune sent as an add, an add rune on a Rare item at its max affix count, a rune on a potion, a player outside the altar's interaction range, and a rune dropped after the enchanting screen opened.
- [ ] Taking damage while the enchanting screen is open closes it without applying the rune.
- [ ] A Workbench crafted from the Wood recipe refunds Wood when deconstructed, and a Workbench crafted from the Stick, Stone, and Fiber recipe refunds Stick, Stone, and Fiber, and the two Workbench items show as separate inventory rows.
- [ ] A crafted item's recipe row ID survives dropping, another player picking it up, and save → quit → load.
- [ ] A Vendor-bought item with no stored recipe row ID uses the first recipe row for its item definition for refunds and repair, and an item whose item definition has no recipe row refunds nothing when deconstructed.
- [ ] A poison crafted by an Alchemist 1 and one crafted by an Alchemist 80 have the same coating buildup and hit count.
...
- `Source/NAMEC/Crafting/NamecEnchantingService.h` — new; affix add/reroll logic, including adding an affix to Common jewelry and raising it to Magic (Requirement 12), and `ServerApplyRune` validation (range, ownership, rune type vs action, affix cap, item eligibility), rune consumption, and no Job XP (Data Flow 9).
- `Source/NAMEC/Crafting/NamecRecipeProvenance.h` — new; stores the recipe row ID on crafted item instances and resolves an item's or placed piece's provenance recipe (stored row, first recipe row for the item definition, or none) for refunds, destroyed-piece drops, and repair (Requirement 24, Edge Case 13).
- `Source/NAMEC/UI/Crafting/` — new; station UI, queue display, hand-crafting menu, dye screen with character preview, and the Enchanting Altar's enchanting screen (Requirement 25).
...
- `Content/Data/DT_Crafting_Rules.uasset` — new; station ranges, queue size, low-level XP penalty, RuneMaxItemLevel per rune tier, Dye Station interaction range, Enchanting Altar interaction range (Requirement 25), and other crafting tuning values.
