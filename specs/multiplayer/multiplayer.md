...
### Session Model
1. The session is a listen server on the host machine using Unreal's legacy replication (not Iris) per `specs/engine-tech/engine-tech.md` Requirement 3. All gameplay state (terrain edits, combat, loot rolls, crafting, progression XP) is server-authoritative.
...
19. Character inventory (including dye colors on item instances), gold, reputation, and quest state are part of the character and leave with the player. World containers, Vendor stock, Quest Board offers, town NPC respawn timers, and camp states belong to the world (`specs/factions-kingdoms/factions-kingdoms.md` Requirements 11, 19, 25, 28, 34, 37, and 48).
...
3. On connection, the client sends `ServerRegisterLocalPlayers(Array<CharacterPayload>)`, where each payload carries the full character state listed for `UNamecCharacterSave` in `specs/game-foundation/game-foundation.md` Requirement 6, including gold, reputation per faction, and quest state → host validates capacity, version, password, and duplicate character GUIDs → spawns pawns and initializes components from the payloads. A local player joining mid-session on a client machine sends the same RPC with one payload (Requirement 4).
4. During play, the server replicates world and gameplay state, including Vendor stock quantities and Quest Board offers, which are world state shared by every player. Owner-only data (loot pickups, full inventory detail, gold, reputation, quest log) replicates only to its owning connection. Per-player loot pickups also render only in the owning local player's viewport, because local players on one machine share a connection (see `specs/combat-loot/combat-loot.md` Requirement 28).
5. On autosave, the host sends each client `ClientSaveCharacter(CharacterPayload)`, including gold, reputation, and quest state → the client writes `UNamecCharacterSave` locally.
...
- [ ] A character from PC B joins PC A's world, gains XP, loot, gold, reputation, and quest progress, disconnects, and the progress is saved on PC B.
- [ ] Stock bought from a Vendor by a player on PC B is reduced for a player on PC A.
...
- `Content/Data/DT_MP_SplitScreenScalability.uasset` — new; High and Split rendering scalability tier rows (`specs/engine-tech/engine-tech.md` Requirement 6).
