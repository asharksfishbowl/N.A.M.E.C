...
12. A shovel removes (digs) or adds (fills) terrain in a sphere of radius 0.5 m (tuning value, scaled by tool tier). A shovel digs only soft materials (soil, sand, gravel, snow, clay) as flagged in `DT_World_VoxelMaterials`. A **fill unit** is the volume of a sphere with the base radius 0.5 m (tuning value), independent of tool tier. Whether a shovel swing digs or fills, and which material it fills with, follow the shovel's Dig/Fill mode and selected fill material (Requirement 40). Filling consumes one item of the selected fill material (Soil, Sand, Gravel, or Stone) per fill unit of volume added, rounded up, places that material, and flags the placed voxels player-placed. When the player holds fewer items of the selected fill material than a fill needs, the fill is blocked (Requirement 42). Digging is limited by the dig depth limit (Requirement 34), and digging and filling are blocked inside town protected radii (Requirement 37). A voxel counts as added when its density crosses the surface threshold from empty to solid, and as removed when it crosses from solid to empty.
...
20. A fallen log is chopped further into Wood items, dropped as world pickups. A stump actor remains at the tree's origin. A shovel swing (Requirement 40) that hits a stump actor, in Dig or Fill mode, removes the stump instead of digging or filling, yields Wood (amount per tree type in `DT_World_Trees`, tuning value) dropped as world pickups like a fallen log's Wood, and awards Woodcutting XP per `DT_Progression_SkillXPSources` (`specs/character-progression/character-progression.md` Requirement 21). Removing a stump is not a terrain edit: it changes no voxels, consumes no fill material, and is not blocked by the dig depth limit or a town protected radius. Whether each harvested tree's stump was removed is saved with the tree harvest state in `UNamecWorldSave` (`specs/game-foundation/game-foundation.md` Requirement 6).
21. Harvested trees regrow after 3 in-game days (tuning value), unless a player has placed a building piece or dug terrain within 3 m of the tree's origin. Removing a stump (Requirement 20) does not count as digging terrain and does not block regrowth. The tree regrows at its stump's origin, and a stump that has not been removed when the tree regrows is replaced by the regrown tree.
...
### Building
24. Building pieces (walls, floors, roofs, stairs, doors, windows, beams, fences) come from crafting (Carpenter/Mason recipes) and are placed from a build menu while holding a Hammer (Requirement 41). Crafting stations, station upgrade attachments, storage containers, beds, training dummies, and other furniture are also building pieces: each is an item placed from the Hammer build menu, has health and a material tier (Requirement 26), and is deconstructed with a Hammer with a material refund (Requirement 27). A placed piece keeps the provenance recipe of the item it was placed from (`specs/crafting-jobs/crafting-jobs.md` Requirement 24), saved with the piece in `UNamecWorldSave`. Deconstructing or destroying a storage container drops its contents per `specs/inventory/inventory.md` Edge Case 7, and deconstructing or destroying a crafting station or attachment follows `specs/crafting-jobs/crafting-jobs.md` Requirement 8 and Edge Case 5.
25. Pieces snap to other pieces' snap points. When no snap point is in range, a piece places freely on terrain.
26. Placed pieces are actors with health and a material tier (wood, stone, reinforced, etc.), not terrain voxels, rendered as Nanite static meshes with destruction visuals per `specs/engine-tech/engine-tech.md` Requirement 3. There is no structural integrity: a piece never collapses from lack of support. Piece health is reduced only by boss attacks, enemy area-of-effect attacks, raider attacks (`specs/factions-kingdoms/factions-kingdoms.md` Requirement 57), and falling tree logs. Player attacks, spells, and area-of-effect abilities never damage building pieces, regardless of the friendly-fire setting (`specs/combat-loot/combat-loot.md` Requirement 12). Training dummies register player hits without losing health (Requirement 44). Only raiders target building pieces. Enemies that are not raiders never target building pieces (`specs/combat-loot/combat-loot.md` Requirement 43). At 0 health a piece is destroyed and drops 50% of its provenance recipe's input materials (tuning value; `specs/crafting-jobs/crafting-jobs.md` Requirement 24). A piece with no provenance recipe drops no materials when destroyed.
27. A Hammer deconstructs any placed piece in the world, placed by any player, and refunds 100% of its provenance recipe's input materials (tuning value; `specs/crafting-jobs/crafting-jobs.md` Requirement 24). A piece with no provenance recipe refunds nothing. Deconstruction happens only through the deconstruct hold in placement mode (Requirements 41 and 43).
...
### Tool Controls
40. A shovel swing (the tool swing input, `specs/game-foundation/game-foundation.md` Requirement 14) digs or fills per the shovel's Dig/Fill mode, which the Dig/Fill toggle input switches, except that a swing that hits a stump actor removes the stump instead (Requirement 20). While a shovel is equipped in the Right Hand, the fill material cycle input cycles the selected fill material through Soil, Sand, Gravel, and Stone, and a fill uses the selected material (Requirement 12). This input replaces the gamepad right-hand and left-hand hotkey cycles only while a shovel is equipped in the Right Hand. The default mode, default fill material, and their persistence follow Requirement 42.
41. While a Hammer is equipped in the Right Hand, the build menu input opens the build menu (Requirement 24). Choosing a piece in the build menu enters placement mode, which shows the placement preview. In placement mode, the place input places the previewed piece (Data Flow 6), the rotate input rotates it, the cancel input ends placement mode, and holding the deconstruct input for 1 s (tuning value in `DT_Core_Input`) while aiming at a placed piece deconstructs that piece (Requirement 27). Placement mode bindings override the hotkey cycles, consumable use, and the other unchorded bindings they share only while placement mode is active (`specs/game-foundation/game-foundation.md` Requirement 14). What happens after a piece is placed follows Requirement 43.

42. A character's shovel Dig/Fill mode and selected fill material (Requirement 40) default to Dig mode with Soil selected. Both belong to the character, not to a shovel item: they apply to every shovel the character equips, and they persist per character in `UNamecCharacterSave` (`specs/game-foundation/game-foundation.md` Requirement 6). When the player makes a fill swing while holding no item of the selected fill material, the swing adds no terrain, and "No <material>" (for example "No Sand") shows in the owning local player's viewport only. When the player holds at least one item of the selected fill material but fewer than the fill needs (Requirement 12), the fill is blocked: no terrain is placed, no item is consumed, and "Not enough <material>" (for example "Not enough Sand") shows in the owning local player's viewport only. In both cases the mode and selected material stay unchanged.
43. Placement mode (Requirement 41) stays active after a piece is placed, with the same piece selected, as long as the player still has at least one item of that piece. When the placement uses the player's last item of that piece, placement mode ends and the build menu opens again. Holding the deconstruct input deconstructs a piece only while placement mode is active: outside placement mode, the same input (gamepad X, keyboard F) uses the selected consumable (`specs/inventory/inventory.md` Requirement 16) and never deconstructs.
...
5. A player chops a tree → server applies damage → on 0 health the server spawns the falling log physics actor (replicated), marks the tree harvested with a regrow timestamp, leaves the stump actor, and awards Woodcutting XP. A player's shovel swing hits the stump → server removes the stump actor without a terrain edit, drops its Wood as world pickups, marks the stump removed in the tree harvest state, and awards Woodcutting XP (Requirement 20).
6. A player places a building piece → server validates snap position, collision, and blocked zones (boss arenas, world spawn point, town protected radii, camp radii), consumes the item, spawns the piece actor with the item's provenance recipe (`specs/crafting-jobs/crafting-jobs.md` Requirement 24), and records it in the world save → the placing client stays in placement mode while the player holds another item of that piece, otherwise returns to the build menu (Requirement 43).
7. On save, the host writes every `UNamecWorldSave` field listed in `specs/game-foundation/game-foundation.md` Requirement 6: world settings, time of day and weather, chunk edit deltas, placed pieces (including each piece's provenance recipe row ID, stations with tiers, containers with contents, and doors with their open or closed state), shared world pickups, tree harvest states (including removed stumps), forage timers, loose pickup collected states and respawn timers, bed respawn points per character GUID, loot chest opened state per character GUID, boss defeat flags, the world's elapsed in-game time, and the town, camp, Vendor stock, and Quest Board state listed there.
...
- [ ] Toggling the shovel to Fill mode and cycling the fill material to Sand makes the next fill consume Sand and place Sand voxels.
- [ ] With a Hammer, RT opens the build menu, choosing a piece enters placement mode, RB places it, and holding X for 1 s on a placed piece deconstructs it with a full refund.
- [ ] With 3 wall items, placing a wall keeps placement mode active with the wall still selected, and placing the third wall returns to the build menu.
- [ ] Outside placement mode, holding X while aiming at a placed piece uses the selected consumable and does not deconstruct the piece.
- [ ] A new character's first equipped shovel is in Dig mode with Soil selected, and a character saved in Fill mode with Gravel selected loads in Fill mode with Gravel selected, on any shovel.
- [ ] A fill swing with no Sand in the inventory while Sand is selected adds no terrain and shows "No Sand" only in that player's viewport.
- [ ] A fill needing 3 Sand with 2 Sand in the inventory places no terrain, consumes no Sand, and shows "Not enough Sand" only in that player's viewport.
- [ ] A Tier 0 Stone Shovel digs Soil and cannot dig Stone.
...
- [ ] A shovel swing on a stump removes it without changing any voxel, drops the Wood amount in `DT_World_Trees`, and awards Woodcutting XP, the tree still regrows at the stump's origin after 3 in-game days, and a removed stump stays removed after save → quit → load.
- [ ] Building pieces snap to each other and refund materials on deconstruction.
- [ ] A Workbench placed from an item crafted with the Wood recipe refunds 100% of that recipe's Wood on deconstruction and drops 50% of it when destroyed by a raider, including after save → quit → load, and a placed piece whose item definition has no recipe row refunds and drops nothing.
- [ ] With friendly fire on, a player's melee attack, spell, and area-of-effect ability each deal 0 damage to a building piece.
- [ ] A player's melee hit, bow shot, and spell on a training dummy each play its hit reaction, leave its piece health unchanged, and award no skill XP, a class ability hit awards no class XP, and a raider attack reduces its health.
...
- `Source/NAMEC/World/NamecTerrainEditComponent.h` — new; dig/fill/mine requests and validation, shovel Dig/Fill mode and selected fill material (Requirement 40), their Dig and Soil defaults, per-character save and load, and the "No <material>" and "Not enough <material>" fill blocks (Requirement 42).
- `Source/NAMEC/World/NamecClimateSubsystem.h` — new; region lookup, day/night, weather.
- `Source/NAMEC/World/NamecTreeActor.h` — new; tree health, felling (swap to a simulating actor with the same static mesh), the stump actor and its shovel removal with Wood yield and Woodcutting XP, and regrowth at the stump's origin (Requirements 20 and 21).
- `Source/NAMEC/World/NamecForageNode.h` — new; harvestable plants.
- `Source/NAMEC/World/Building/NamecBuildPiece.h` — new; placeable piece actor with snap points, health, and its provenance recipe row ID, used for the destroyed-piece drop and deconstruct refund (Requirements 24, 26, and 27).
...
- `Content/Data/DT_World_Trees.uasset` — new; tree types, health, yields, including the Wood yield of a removed stump per tree type (Requirement 20).
