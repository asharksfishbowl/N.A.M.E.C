...
- Item stacking limits. Any quantity stacks in one row per item definition plus potency value, so consumables with different potency values are separate rows (Edge Case 11). Items whose instances differ in other ways, such as dye colors, also show as separate rows (Edge Case 8).
...
4. On save, the inventory component serializes every item instance (definition ID, quantity, rarity, affixes, durability (none for jewelry), item level, dye colors, and potency value for consumables), equipped slots, favorites, hotkeys, and sort choice into `UNamecCharacterSave`, and `UNamecCurrencyComponent` serializes gold (`specs/factions-kingdoms/factions-kingdoms.md` Requirement 28).
...
## Key Files
- `Source/NAMEC/Inventory/NamecInventoryComponent.h` — new; item storage with one row per item definition plus potency value for stackable items (Edge Case 11), weight, equip slots including Cloak, Neck, and Ring (Requirements 25 and 26), favorites, hotkeys.
- `Source/NAMEC/Inventory/NamecItemDefinition.h` — new; item definition data asset (weight, `Value`, category, slot, armor category, weapon category (`specs/character-progression/character-progression.md` Requirement 29), `Poise` for armor, `PoiseDamage` for weapons, `WeaponBase` for weapons and tools (`specs/combat-loot/combat-loot.md` Requirement 56), `ArrowDamage` for arrows (`specs/combat-loot/combat-loot.md` Requirement 53), Insulation or Cooling for capes (Requirement 25), the no-durability flag and no base stats for jewelry (Requirement 26), dye zones and material mask channels, quest item flag, effects; see `specs/combat-loot/combat-loot.md` Requirement 38 and `specs/crafting-jobs/crafting-jobs.md` Requirement 20).
