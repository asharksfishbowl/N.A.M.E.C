...
| `specs/voxel-world/voxel-world.md` | World layout, climates, terrain dig/fill, building, gathering |
| `specs/authored-map/authored-map.md` | The one authored map: map asset, bake tool and stroke log, map validator, authored structure actors, save map identity and load rule, expansion placeholders, repository and Phase 1 handover shape |
...
- **World** — one play-through of the shipped authored map (`specs/authored-map/authored-map.md`) plus all terrain edits, placed structures, containers, and boss-defeat state. Saved on the host machine.
...
14. All input uses Enhanced Input. Every binding below is the default and is remappable per local player in Settings, and remaps are saved per local player slot in `UNamecSettingsSave` (Requirement 6). Remapping an action onto a key that another action in the same mapping context already uses swaps the two bindings for that local player slot: the displaced action takes the remapped action's previous key, so no action is ever left unbound (Director decision 2026-09-19 under user approval; refuse and allow-duplicates were the alternatives). Chorded bindings (LB held + a button, Left Alt + RMB) take priority over the unchorded binding of the same button, so an unchorded action never fires while its chord modifier is held. While an execution prompt is shown in a local player's viewport (`specs/combat-loot/combat-loot.md` Requirement 45), that player's unchorded Y (gamepad) or R (keyboard) executes instead of toggling two-handing; LB + Y still casts ability 4. Context bindings replace the unchorded default binding of the same button only while their context is active, and chord priority still applies inside every context. Interact targets include doors, Quest Boards, NPCs, containers, pickups and the `ANamecTerrainUpdatedMarker` (`specs/authored-map/authored-map.md` Requirement 25):
...
2. Player selects New World (enters world name and world settings per `specs/voxel-world/voxel-world.md` Requirement 5; there is no seed or size input) or Load World (`specs/authored-map/authored-map.md` Requirement 25 load rule) → the lobby opens (`specs/multiplayer/multiplayer.md` Requirement 4). Local player 1 picks a character from this machine's saves or creates one with "Create New" (`specs/character-creation/character-creation.md` Requirement 1), and additional local players press Start to join and each pick or create a character → each chosen `UNamecCharacterSave` loads into memory → host loads `UNamecWorldSave` and streams terrain from the map asset, applying saved terrain edits per chunk (`specs/authored-map/authored-map.md` Requirement 29).
...
- [ ] A character created in World A can be loaded into World B with race, sex, appearance, stats, classes, skills, jobs, inventory (including dye colors, provenance recipe row IDs, and weapon coatings with their remaining hits), gold, reputation, and quest state intact.
...
- `Source/NAMEC/Save/NamecSettingsSave.h` — new; machine-local settings save object with `SaveVersion`, one section per local player slot index (1–4) and one machine-wide section holding audio and the graphics settings that cap the Split rendering tier (Requirement 6), with the `.bak` rename for a corrupt file and session-only defaults for a newer `SaveVersion` (Requirement 7, Edge Case 1).
