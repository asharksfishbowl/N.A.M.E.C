...
- XP per use = base XP × difficulty multiplier. The multiplier comes from the target's tier: ore tier, tree tier, or enemy level relative to your character level.
- Each skill level gives a passive bonus to its action (for example, Mining raises dig speed and ore yield chance).
- 25% (starting value, tunable) of all skill XP also goes to character XP. See [Stats and Classes](Stats-and-Classes.md).
...
| Skill | Earns XP for |
|-------|--------------|
| One-Handed | Each hit that damages a hostile enemy |
...
- **Climbing:** any terrain or building surface steeper than 60° can be climbed by holding jump against it. Climbing drains stamina per second, and at 0 stamina you fall.
- **Stealth:** each enemy has its own perception radius. Crouching halves it, and Stealth skill shrinks it further. "Undetected" means the enemy has not perceived or aggroed you.
